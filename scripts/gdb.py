...
def sgx_load_sym_file(file_name, tcs_addr):
   offset=get_text_offset(file_name)
   address=find_vma_base(tcs_addr) + offset
...
class Sgx_Add_Symbol_File (gdb.Command):
  """
  Given TCS address and ELF path, load enclave's symbols.
  sgx-add-symbol-file <TCS_ADDR | auto> <ELF_PATH>
  if tcs address as auto indicates that tcs address should be read from rbx.
  """
  ...
  def invoke (self, arg, from_tty):
    ...
    if (args[0] == 'auto'):
        tcs=gdb.parse_and_eval("$rbx")
    else:
        tcs=int(args[0])
    file_path=args[1]
    sgx_load_sym_file(file_path, tcs)
